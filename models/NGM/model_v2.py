...
class Net(CNN):
    def __init__(self):
        ...
        if cfg.ATTN.SWITCH == True:
            self.attn = TransformerConvLayer(cfg.ATTN.IN_DIM, cfg.ATTN.OUT_DIM, depth=cfg.ATTN.DEPTH,
                                             head=cfg.ATTN.HEAD, sk_channel=cfg.ATTN.SK_EMB, sk_tau=cfg.NGM.SK_TAU)
        else:
            self.gnn_layer = cfg.NGM.GNN_LAYER
