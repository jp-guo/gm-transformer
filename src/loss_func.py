...
class EntropyPermutationLoss(nn.Module):
    def __init__(self, gamma=1.):
        super(EntropyPermutationLoss, self).__init__()
        self.gamma = gamma
    ...
    def forward(self, pred_dsmat: Tensor, gt_perm: Tensor, src_ns: Tensor, tgt_ns: Tensor) -> Tensor:
        ...
        for b in range(batch_num):
            ...
            #     pred_dsmat[batch_slice],
            #     gt_perm[batch_slice],
            #     reduction='sum'), (pred_dsmat[batch_slice] * torch.log(pred_dsmat[batch_slice])).sum() * self.gamma)
            loss = loss + (pred_dsmat[batch_slice] * torch.log(pred_dsmat[batch_slice] + 1.e-9)).sum() * self.gamma # information entropy
...
class EntropyLoss(nn.Module):
    ...
    def forward(self, pred_dsmat: Tensor, gt_perm: Tensor, src_ns: Tensor, tgt_ns: Tensor) -> Tensor:
        ...
        for b in range(batch_num):
            ...
            #     gt_index,
            #     reduction='sum'), (pred_dsmat[batch_slice] * torch.log(pred_dsmat[batch_slice])).sum() * self.gamma)
            loss = loss + (pred_dsmat[batch_slice] * torch.log(pred_dsmat[batch_slice])).sum() * self.gamma
            n_sum += src_ns[b].to(n_sum.dtype).to(pred_dsmat.device)
