...
class CeIT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        return nodes, edges, last_cls_token.view(B, -1)

    def forward(self, x):
        nodes, edges, glb = self.forward_features(x)

        nodes, edges = nodes.transpose(1, 2), edges.transpose(1, 2)

        nodes, edges = nodes.reshape((nodes.shape[0], nodes.shape[1], self.num_patch, self.num_patch)), \
                       edges.reshape((edges.shape[0], edges.shape[1], self.num_patch, self.num_patch))
